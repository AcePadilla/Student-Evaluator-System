...
if(isset($_SESSION['username'])) {
    ...
    $teacher_subject_query = "SELECT subject_code FROM teachers WHERE Username='$teacher_username'";
    $teacher_subject_result = mysqli_query($conn, $teacher_subject_query);
    $teacher_subjects = [];
    while ($row = mysqli_fetch_assoc($teacher_subject_result)) {
        $teacher_subjects[] = $row["subject_code"];
    }
} else {
    $teacher_subjects = [];  // If no teacher is logged in, initialize an empty array.
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($teacher_subjects) || in_array($subject_code, $teacher_subjects)) {
        ...
        $remarks = ($average_grade >= 50) ? "Passed" : "Failed";
        if ($remarks == "Failed") {
            $any_fail = true;
        }
...
<body>
    ...
    ?>
    <input type="number" name="prelim_grade_<?= $enrollment["subject_code"] ?>" value="<?= $enrollment["prelim_grade"] ?>" required></td>
                            <td>
    <?php 
    if (!empty($teacher_subjects) && !in_array($subject_code, $teacher_subjects)) {
        echo "disabled";
    ...
    ?>
    <input type="number" name="midterm_grade_<?= $enrollment["subject_code"] ?>" value="<?= $enrollment["midterm_grade"] ?>" required></td>
                            <td>
    <?php 
    if (!empty($teacher_subjects) && !in_array($subject_code, $teacher_subjects)) {
        echo "disabled";
    ...
    ?>
    <input type="number" name="final_grade_<?= $enrollment["subject_code"] ?>" value="<?= $enrollment["final_grade"] ?>" required></td>
                            <td>
    <?php 
    if (!empty($teacher_subjects) && !in_array($subject_code, $teacher_subjects)) {
        echo "disabled";
    ...
    ?>
    <input type="number" name="average_<?= $enrollment["subject_code"] ?>" value="<?= $enrollment["average"] ?>" readonly></td>
